<script>
import AppHeader from './components/AppHeader.vue';
import AppHero from './components/AppHero.vue';
import AppCards from './components/AppCards.vue';

export default {
    name: 'App',
    components: {
        AppHeader,
        AppHero,
        AppCards,
    }
}
</script>

<template>
    <AppHeader />
    <AppHero />
    <AppCards />

    <main>
        <div id="who_we_are_section" class="p-5">
            <div class="container-fluid align-items-center d-flex flex-column py-5">
                <div class="">
                    <h6>Who We Are</h6>
                    <h1 class="">Sense the jazz</h1>
                    <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Tenetur et expedita dignissimos? Illum
                        debitis dolores dicta quam doloribus inventore error unde accusantium, architecto vel sapiente quis
                        expedita hic provident omnis. Lorem ipsum dolor, sit amet consectetur adipisicing elit. Natus illum
                        eveniet iure sapiente, sunt totam, sed earum, soluta minima maxime nihil quibusdam! Quos voluptate
                        eaque impedit eum minima maxime ut. Lorem ipsum dolor sit amet consectetur, adipisicing elit. Eos
                        omnis, odit facere voluptatibus illo tempora aperiam soluta eaque error, culpa expedita, vel iure
                        dolores sunt porro earum? Eaque, deserunt tempore.</p>
                </div>
                <div>
                    <button type="button" class="btn btn_cstm btn-lg">read more</button>
                </div>

            </div>



        </div>

        <div id="video_section" class="p-5">
            <div class="container-fluid align-items-center d-flex flex-column py-5">
                <div><img src="./assets/svg/image (2).svg" alt=""></div>
                <div class="play orange_filter">
                    <svg xmlns="http://www.w3.org/2000/svg" width="48" class="bi bi-play-circle" viewBox="0 0 16 16">
                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z" />
                        <path
                            d="M6.271 5.055a.5.5 0 0 1 .52.038l3.5 2.5a.5.5 0 0 1 0 .814l-3.5 2.5A.5.5 0 0 1 6 10.5v-5a.5.5 0 0 1 .271-.445z" />
                    </svg>
                </div>
                <h1 class="">New Music Video</h1>

            </div>

        </div>

        <div id="gallery_section" class="p-5">
            <div class="container align-items-center d-flex flex-column py-5">
                <h6>Who We Are</h6>
                <h1 class="">Sense the jazz</h1>
                <div class="row justify-content-space-around align-items-center g-4 py-1">
                    <div class="col-4">

                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/gallery1.jpg" alt="">

                            <div class="card-body">
                                <h5 class="card-title">A roll and bang the floor</h5>
                                <div class="card-text d-flex">Stream On:
                                    <span class="d-flex">

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" class="bi bi-spotify"
                                            viewBox="0 0 16 16">
                                            <path
                                                d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm3.669 11.538a.498.498 0 0 1-.686.165c-1.879-1.147-4.243-1.407-7.028-.77a.499.499 0 0 1-.222-.973c3.048-.696 5.662-.397 7.77.892a.5.5 0 0 1 .166.686zm.979-2.178a.624.624 0 0 1-.858.205c-2.15-1.321-5.428-1.704-7.972-.932a.625.625 0 0 1-.362-1.194c2.905-.881 6.517-.454 8.986 1.063a.624.624 0 0 1 .206.858zm.084-2.268C10.154 5.56 5.9 5.419 3.438 6.166a.748.748 0 1 1-.434-1.432c2.825-.857 7.523-.692 10.492 1.07a.747.747 0 1 1-.764 1.288z" />
                                        </svg>
                                        <!-- /spotify icn -->

                                        <svg class="thebase-svg-icon thebase-youtube-svg" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 28 28">
                                            <title>YouTube</title>
                                            <path
                                                d="M11.109 17.625l7.562-3.906-7.562-3.953v7.859zM14 4.156c5.891 0 9.797 0.281 9.797 0.281 0.547 0.063 1.75 0.063 2.812 1.188 0 0 0.859 0.844 1.109 2.781 0.297 2.266 0.281 4.531 0.281 4.531v2.125s0.016 2.266-0.281 4.531c-0.25 1.922-1.109 2.781-1.109 2.781-1.062 1.109-2.266 1.109-2.812 1.172 0 0-3.906 0.297-9.797 0.297v0c-7.281-0.063-9.516-0.281-9.516-0.281-0.625-0.109-2.031-0.078-3.094-1.188 0 0-0.859-0.859-1.109-2.781-0.297-2.266-0.281-4.531-0.281-4.531v-2.125s-0.016-2.266 0.281-4.531c0.25-1.937 1.109-2.781 1.109-2.781 1.062-1.125 2.266-1.125 2.812-1.188 0 0 3.906-0.281 9.797-0.281v0z">
                                            </path>
                                        </svg>
                                        <!-- /youtube icn -->

                                        <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 372.12 396.03">
                                            <path
                                                d="M370.43,0H372q0,67.4,0,134.8c0,48.13.13,96.26,0,144.39-.15,42.61-34.59,75-76.53,72.25a71.27,71.27,0,0,1-65.93-81.9c4.09-26.63,19-45.65,43.77-55.77,25.61-10.44,49.86-6.43,71.91,10.38.76.58,1.6,1.08,2.64,1.77v-117c-1.46.3-2.75.51-4,.84q-101.61,25.77-203.2,51.56c-6.11,1.55-6.1,1.58-6.1,8,0,55.46-.08,110.93,0,166.39,0,17.59-4.82,33.42-16,47.05-3.91,4.78-8.67,8.87-13,13.27H20.8c-8.53-6.29-15.21-14.26-20.8-23.2v-70.4a25,25,0,0,0,2.19-2.67C20.84,269.92,59,257.92,91.11,272.12c6.7,3,12.83,7.23,19.36,11V66.49c1.73-.52,3.23-1,4.76-1.41Q227.84,36.53,340.46,8C350.48,5.46,360.44,2.67,370.43,0Z">
                                            </path>
                                        </svg>
                                        <!-- /note icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 640 512">
                                            <path
                                                d="M111.4 256.3l5.8 65-5.8 68.3c-.3 2.5-2.2 4.4-4.4 4.4s-4.2-1.9-4.2-4.4l-5.6-68.3 5.6-65c0-2.2 1.9-4.2 4.2-4.2 2.2 0 4.1 2 4.4 4.2zm21.4-45.6c-2.8 0-4.7 2.2-5 5l-5 105.6 5 68.3c.3 2.8 2.2 5 5 5 2.5 0 4.7-2.2 4.7-5l5.8-68.3-5.8-105.6c0-2.8-2.2-5-4.7-5zm25.5-24.1c-3.1 0-5.3 2.2-5.6 5.3l-4.4 130 4.4 67.8c.3 3.1 2.5 5.3 5.6 5.3 2.8 0 5.3-2.2 5.3-5.3l5.3-67.8-5.3-130c0-3.1-2.5-5.3-5.3-5.3zM7.2 283.2c-1.4 0-2.2 1.1-2.5 2.5L0 321.3l4.7 35c.3 1.4 1.1 2.5 2.5 2.5s2.2-1.1 2.5-2.5l5.6-35-5.6-35.6c-.3-1.4-1.1-2.5-2.5-2.5zm23.6-21.9c-1.4 0-2.5 1.1-2.5 2.5l-6.4 57.5 6.4 56.1c0 1.7 1.1 2.8 2.5 2.8s2.5-1.1 2.8-2.5l7.2-56.4-7.2-57.5c-.3-1.4-1.4-2.5-2.8-2.5zm25.3-11.4c-1.7 0-3.1 1.4-3.3 3.3L47 321.3l5.8 65.8c.3 1.7 1.7 3.1 3.3 3.1 1.7 0 3.1-1.4 3.1-3.1l6.9-65.8-6.9-68.1c0-1.9-1.4-3.3-3.1-3.3zm25.3-2.2c-1.9 0-3.6 1.4-3.6 3.6l-5.8 70 5.8 67.8c0 2.2 1.7 3.6 3.6 3.6s3.6-1.4 3.9-3.6l6.4-67.8-6.4-70c-.3-2.2-2-3.6-3.9-3.6zm241.4-110.9c-1.1-.8-2.8-1.4-4.2-1.4-2.2 0-4.2.8-5.6 1.9-1.9 1.7-3.1 4.2-3.3 6.7v.8l-3.3 176.7 1.7 32.5 1.7 31.7c.3 4.7 4.2 8.6 8.9 8.6s8.6-3.9 8.6-8.6l3.9-64.2-3.9-177.5c-.4-3-2-5.8-4.5-7.2zm-26.7 15.3c-1.4-.8-2.8-1.4-4.4-1.4s-3.1.6-4.4 1.4c-2.2 1.4-3.6 3.9-3.6 6.7l-.3 1.7-2.8 160.8s0 .3 3.1 65.6v.3c0 1.7.6 3.3 1.7 4.7 1.7 1.9 3.9 3.1 6.4 3.1 2.2 0 4.2-1.1 5.6-2.5 1.7-1.4 2.5-3.3 2.5-5.6l.3-6.7 3.1-58.6-3.3-162.8c-.3-2.8-1.7-5.3-3.9-6.7zm-111.4 22.5c-3.1 0-5.8 2.8-5.8 6.1l-4.4 140.6 4.4 67.2c.3 3.3 2.8 5.8 5.8 5.8 3.3 0 5.8-2.5 6.1-5.8l5-67.2-5-140.6c-.2-3.3-2.7-6.1-6.1-6.1zm376.7 62.8c-10.8 0-21.1 2.2-30.6 6.1-6.4-70.8-65.8-126.4-138.3-126.4-17.8 0-35 3.3-50.3 9.4-6.1 2.2-7.8 4.4-7.8 9.2v249.7c0 5 3.9 8.6 8.6 9.2h218.3c43.3 0 78.6-35 78.6-78.3.1-43.6-35.2-78.9-78.5-78.9zm-296.7-60.3c-4.2 0-7.5 3.3-7.8 7.8l-3.3 136.7 3.3 65.6c.3 4.2 3.6 7.5 7.8 7.5 4.2 0 7.5-3.3 7.5-7.5l3.9-65.6-3.9-136.7c-.3-4.5-3.3-7.8-7.5-7.8zm-53.6-7.8c-3.3 0-6.4 3.1-6.4 6.7l-3.9 145.3 3.9 66.9c.3 3.6 3.1 6.4 6.4 6.4 3.6 0 6.4-2.8 6.7-6.4l4.4-66.9-4.4-145.3c-.3-3.6-3.1-6.7-6.7-6.7zm26.7 3.4c-3.9 0-6.9 3.1-6.9 6.9L227 321.3l3.9 66.4c.3 3.9 3.1 6.9 6.9 6.9s6.9-3.1 6.9-6.9l4.2-66.4-4.2-141.7c0-3.9-3-6.9-6.9-6.9z" />
                                        </svg>
                                        <!-- /soundcloud icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 512 512">
                                            <path
                                                d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm50.7-186.9L162.4 380.6c-19.4 7.5-38.5-11.6-31-31l55.5-144.3c3.3-8.5 9.9-15.1 18.4-18.4l144.3-55.5c19.4-7.5 38.5 11.6 31 31L325.1 306.7c-3.2 8.5-9.9 15.1-18.4 18.4zM288 256a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" />
                                        </svg>
                                        <!-- /compass icn -->


                                    </span>


                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="col-4">

                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/gallery2.jpg" alt="">

                            <div class="card-body">
                                <h5 class="card-title">Your melody to our music</h5>
                                <div class="card-text d-flex">Stream On:
                                    <span class="d-flex">

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" class="bi bi-spotify"
                                            viewBox="0 0 16 16">
                                            <path
                                                d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm3.669 11.538a.498.498 0 0 1-.686.165c-1.879-1.147-4.243-1.407-7.028-.77a.499.499 0 0 1-.222-.973c3.048-.696 5.662-.397 7.77.892a.5.5 0 0 1 .166.686zm.979-2.178a.624.624 0 0 1-.858.205c-2.15-1.321-5.428-1.704-7.972-.932a.625.625 0 0 1-.362-1.194c2.905-.881 6.517-.454 8.986 1.063a.624.624 0 0 1 .206.858zm.084-2.268C10.154 5.56 5.9 5.419 3.438 6.166a.748.748 0 1 1-.434-1.432c2.825-.857 7.523-.692 10.492 1.07a.747.747 0 1 1-.764 1.288z" />
                                        </svg>
                                        <!-- /spotify icn -->

                                        <svg class="thebase-svg-icon thebase-youtube-svg" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 28 28">
                                            <title>YouTube</title>
                                            <path
                                                d="M11.109 17.625l7.562-3.906-7.562-3.953v7.859zM14 4.156c5.891 0 9.797 0.281 9.797 0.281 0.547 0.063 1.75 0.063 2.812 1.188 0 0 0.859 0.844 1.109 2.781 0.297 2.266 0.281 4.531 0.281 4.531v2.125s0.016 2.266-0.281 4.531c-0.25 1.922-1.109 2.781-1.109 2.781-1.062 1.109-2.266 1.109-2.812 1.172 0 0-3.906 0.297-9.797 0.297v0c-7.281-0.063-9.516-0.281-9.516-0.281-0.625-0.109-2.031-0.078-3.094-1.188 0 0-0.859-0.859-1.109-2.781-0.297-2.266-0.281-4.531-0.281-4.531v-2.125s-0.016-2.266 0.281-4.531c0.25-1.937 1.109-2.781 1.109-2.781 1.062-1.125 2.266-1.125 2.812-1.188 0 0 3.906-0.281 9.797-0.281v0z">
                                            </path>
                                        </svg>
                                        <!-- /youtube icn -->

                                        <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 372.12 396.03">
                                            <path
                                                d="M370.43,0H372q0,67.4,0,134.8c0,48.13.13,96.26,0,144.39-.15,42.61-34.59,75-76.53,72.25a71.27,71.27,0,0,1-65.93-81.9c4.09-26.63,19-45.65,43.77-55.77,25.61-10.44,49.86-6.43,71.91,10.38.76.58,1.6,1.08,2.64,1.77v-117c-1.46.3-2.75.51-4,.84q-101.61,25.77-203.2,51.56c-6.11,1.55-6.1,1.58-6.1,8,0,55.46-.08,110.93,0,166.39,0,17.59-4.82,33.42-16,47.05-3.91,4.78-8.67,8.87-13,13.27H20.8c-8.53-6.29-15.21-14.26-20.8-23.2v-70.4a25,25,0,0,0,2.19-2.67C20.84,269.92,59,257.92,91.11,272.12c6.7,3,12.83,7.23,19.36,11V66.49c1.73-.52,3.23-1,4.76-1.41Q227.84,36.53,340.46,8C350.48,5.46,360.44,2.67,370.43,0Z">
                                            </path>
                                        </svg>
                                        <!-- /note icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 640 512">
                                            <path
                                                d="M111.4 256.3l5.8 65-5.8 68.3c-.3 2.5-2.2 4.4-4.4 4.4s-4.2-1.9-4.2-4.4l-5.6-68.3 5.6-65c0-2.2 1.9-4.2 4.2-4.2 2.2 0 4.1 2 4.4 4.2zm21.4-45.6c-2.8 0-4.7 2.2-5 5l-5 105.6 5 68.3c.3 2.8 2.2 5 5 5 2.5 0 4.7-2.2 4.7-5l5.8-68.3-5.8-105.6c0-2.8-2.2-5-4.7-5zm25.5-24.1c-3.1 0-5.3 2.2-5.6 5.3l-4.4 130 4.4 67.8c.3 3.1 2.5 5.3 5.6 5.3 2.8 0 5.3-2.2 5.3-5.3l5.3-67.8-5.3-130c0-3.1-2.5-5.3-5.3-5.3zM7.2 283.2c-1.4 0-2.2 1.1-2.5 2.5L0 321.3l4.7 35c.3 1.4 1.1 2.5 2.5 2.5s2.2-1.1 2.5-2.5l5.6-35-5.6-35.6c-.3-1.4-1.1-2.5-2.5-2.5zm23.6-21.9c-1.4 0-2.5 1.1-2.5 2.5l-6.4 57.5 6.4 56.1c0 1.7 1.1 2.8 2.5 2.8s2.5-1.1 2.8-2.5l7.2-56.4-7.2-57.5c-.3-1.4-1.4-2.5-2.8-2.5zm25.3-11.4c-1.7 0-3.1 1.4-3.3 3.3L47 321.3l5.8 65.8c.3 1.7 1.7 3.1 3.3 3.1 1.7 0 3.1-1.4 3.1-3.1l6.9-65.8-6.9-68.1c0-1.9-1.4-3.3-3.1-3.3zm25.3-2.2c-1.9 0-3.6 1.4-3.6 3.6l-5.8 70 5.8 67.8c0 2.2 1.7 3.6 3.6 3.6s3.6-1.4 3.9-3.6l6.4-67.8-6.4-70c-.3-2.2-2-3.6-3.9-3.6zm241.4-110.9c-1.1-.8-2.8-1.4-4.2-1.4-2.2 0-4.2.8-5.6 1.9-1.9 1.7-3.1 4.2-3.3 6.7v.8l-3.3 176.7 1.7 32.5 1.7 31.7c.3 4.7 4.2 8.6 8.9 8.6s8.6-3.9 8.6-8.6l3.9-64.2-3.9-177.5c-.4-3-2-5.8-4.5-7.2zm-26.7 15.3c-1.4-.8-2.8-1.4-4.4-1.4s-3.1.6-4.4 1.4c-2.2 1.4-3.6 3.9-3.6 6.7l-.3 1.7-2.8 160.8s0 .3 3.1 65.6v.3c0 1.7.6 3.3 1.7 4.7 1.7 1.9 3.9 3.1 6.4 3.1 2.2 0 4.2-1.1 5.6-2.5 1.7-1.4 2.5-3.3 2.5-5.6l.3-6.7 3.1-58.6-3.3-162.8c-.3-2.8-1.7-5.3-3.9-6.7zm-111.4 22.5c-3.1 0-5.8 2.8-5.8 6.1l-4.4 140.6 4.4 67.2c.3 3.3 2.8 5.8 5.8 5.8 3.3 0 5.8-2.5 6.1-5.8l5-67.2-5-140.6c-.2-3.3-2.7-6.1-6.1-6.1zm376.7 62.8c-10.8 0-21.1 2.2-30.6 6.1-6.4-70.8-65.8-126.4-138.3-126.4-17.8 0-35 3.3-50.3 9.4-6.1 2.2-7.8 4.4-7.8 9.2v249.7c0 5 3.9 8.6 8.6 9.2h218.3c43.3 0 78.6-35 78.6-78.3.1-43.6-35.2-78.9-78.5-78.9zm-296.7-60.3c-4.2 0-7.5 3.3-7.8 7.8l-3.3 136.7 3.3 65.6c.3 4.2 3.6 7.5 7.8 7.5 4.2 0 7.5-3.3 7.5-7.5l3.9-65.6-3.9-136.7c-.3-4.5-3.3-7.8-7.5-7.8zm-53.6-7.8c-3.3 0-6.4 3.1-6.4 6.7l-3.9 145.3 3.9 66.9c.3 3.6 3.1 6.4 6.4 6.4 3.6 0 6.4-2.8 6.7-6.4l4.4-66.9-4.4-145.3c-.3-3.6-3.1-6.7-6.7-6.7zm26.7 3.4c-3.9 0-6.9 3.1-6.9 6.9L227 321.3l3.9 66.4c.3 3.9 3.1 6.9 6.9 6.9s6.9-3.1 6.9-6.9l4.2-66.4-4.2-141.7c0-3.9-3-6.9-6.9-6.9z" />
                                        </svg>
                                        <!-- /soundcloud icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 512 512">
                                            <path
                                                d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm50.7-186.9L162.4 380.6c-19.4 7.5-38.5-11.6-31-31l55.5-144.3c3.3-8.5 9.9-15.1 18.4-18.4l144.3-55.5c19.4-7.5 38.5 11.6 31 31L325.1 306.7c-3.2 8.5-9.9 15.1-18.4 18.4zM288 256a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" />
                                        </svg>
                                        <!-- /compass icn -->


                                    </span>


                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="col-4">

                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/gallery3.jpg" alt="">

                            <div class="card-body">
                                <h5 class="card-title">touching heaven on frequency</h5>
                                <div class="card-text d-flex">Stream On:
                                    <span class="d-flex">

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" class="bi bi-spotify"
                                            viewBox="0 0 16 16">
                                            <path
                                                d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm3.669 11.538a.498.498 0 0 1-.686.165c-1.879-1.147-4.243-1.407-7.028-.77a.499.499 0 0 1-.222-.973c3.048-.696 5.662-.397 7.77.892a.5.5 0 0 1 .166.686zm.979-2.178a.624.624 0 0 1-.858.205c-2.15-1.321-5.428-1.704-7.972-.932a.625.625 0 0 1-.362-1.194c2.905-.881 6.517-.454 8.986 1.063a.624.624 0 0 1 .206.858zm.084-2.268C10.154 5.56 5.9 5.419 3.438 6.166a.748.748 0 1 1-.434-1.432c2.825-.857 7.523-.692 10.492 1.07a.747.747 0 1 1-.764 1.288z" />
                                        </svg>
                                        <!-- /spotify icn -->

                                        <svg class="thebase-svg-icon thebase-youtube-svg" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 28 28">
                                            <title>YouTube</title>
                                            <path
                                                d="M11.109 17.625l7.562-3.906-7.562-3.953v7.859zM14 4.156c5.891 0 9.797 0.281 9.797 0.281 0.547 0.063 1.75 0.063 2.812 1.188 0 0 0.859 0.844 1.109 2.781 0.297 2.266 0.281 4.531 0.281 4.531v2.125s0.016 2.266-0.281 4.531c-0.25 1.922-1.109 2.781-1.109 2.781-1.062 1.109-2.266 1.109-2.812 1.172 0 0-3.906 0.297-9.797 0.297v0c-7.281-0.063-9.516-0.281-9.516-0.281-0.625-0.109-2.031-0.078-3.094-1.188 0 0-0.859-0.859-1.109-2.781-0.297-2.266-0.281-4.531-0.281-4.531v-2.125s-0.016-2.266 0.281-4.531c0.25-1.937 1.109-2.781 1.109-2.781 1.062-1.125 2.266-1.125 2.812-1.188 0 0 3.906-0.281 9.797-0.281v0z">
                                            </path>
                                        </svg>
                                        <!-- /youtube icn -->

                                        <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 372.12 396.03">
                                            <path
                                                d="M370.43,0H372q0,67.4,0,134.8c0,48.13.13,96.26,0,144.39-.15,42.61-34.59,75-76.53,72.25a71.27,71.27,0,0,1-65.93-81.9c4.09-26.63,19-45.65,43.77-55.77,25.61-10.44,49.86-6.43,71.91,10.38.76.58,1.6,1.08,2.64,1.77v-117c-1.46.3-2.75.51-4,.84q-101.61,25.77-203.2,51.56c-6.11,1.55-6.1,1.58-6.1,8,0,55.46-.08,110.93,0,166.39,0,17.59-4.82,33.42-16,47.05-3.91,4.78-8.67,8.87-13,13.27H20.8c-8.53-6.29-15.21-14.26-20.8-23.2v-70.4a25,25,0,0,0,2.19-2.67C20.84,269.92,59,257.92,91.11,272.12c6.7,3,12.83,7.23,19.36,11V66.49c1.73-.52,3.23-1,4.76-1.41Q227.84,36.53,340.46,8C350.48,5.46,360.44,2.67,370.43,0Z">
                                            </path>
                                        </svg>
                                        <!-- /note icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 640 512">
                                            <path
                                                d="M111.4 256.3l5.8 65-5.8 68.3c-.3 2.5-2.2 4.4-4.4 4.4s-4.2-1.9-4.2-4.4l-5.6-68.3 5.6-65c0-2.2 1.9-4.2 4.2-4.2 2.2 0 4.1 2 4.4 4.2zm21.4-45.6c-2.8 0-4.7 2.2-5 5l-5 105.6 5 68.3c.3 2.8 2.2 5 5 5 2.5 0 4.7-2.2 4.7-5l5.8-68.3-5.8-105.6c0-2.8-2.2-5-4.7-5zm25.5-24.1c-3.1 0-5.3 2.2-5.6 5.3l-4.4 130 4.4 67.8c.3 3.1 2.5 5.3 5.6 5.3 2.8 0 5.3-2.2 5.3-5.3l5.3-67.8-5.3-130c0-3.1-2.5-5.3-5.3-5.3zM7.2 283.2c-1.4 0-2.2 1.1-2.5 2.5L0 321.3l4.7 35c.3 1.4 1.1 2.5 2.5 2.5s2.2-1.1 2.5-2.5l5.6-35-5.6-35.6c-.3-1.4-1.1-2.5-2.5-2.5zm23.6-21.9c-1.4 0-2.5 1.1-2.5 2.5l-6.4 57.5 6.4 56.1c0 1.7 1.1 2.8 2.5 2.8s2.5-1.1 2.8-2.5l7.2-56.4-7.2-57.5c-.3-1.4-1.4-2.5-2.8-2.5zm25.3-11.4c-1.7 0-3.1 1.4-3.3 3.3L47 321.3l5.8 65.8c.3 1.7 1.7 3.1 3.3 3.1 1.7 0 3.1-1.4 3.1-3.1l6.9-65.8-6.9-68.1c0-1.9-1.4-3.3-3.1-3.3zm25.3-2.2c-1.9 0-3.6 1.4-3.6 3.6l-5.8 70 5.8 67.8c0 2.2 1.7 3.6 3.6 3.6s3.6-1.4 3.9-3.6l6.4-67.8-6.4-70c-.3-2.2-2-3.6-3.9-3.6zm241.4-110.9c-1.1-.8-2.8-1.4-4.2-1.4-2.2 0-4.2.8-5.6 1.9-1.9 1.7-3.1 4.2-3.3 6.7v.8l-3.3 176.7 1.7 32.5 1.7 31.7c.3 4.7 4.2 8.6 8.9 8.6s8.6-3.9 8.6-8.6l3.9-64.2-3.9-177.5c-.4-3-2-5.8-4.5-7.2zm-26.7 15.3c-1.4-.8-2.8-1.4-4.4-1.4s-3.1.6-4.4 1.4c-2.2 1.4-3.6 3.9-3.6 6.7l-.3 1.7-2.8 160.8s0 .3 3.1 65.6v.3c0 1.7.6 3.3 1.7 4.7 1.7 1.9 3.9 3.1 6.4 3.1 2.2 0 4.2-1.1 5.6-2.5 1.7-1.4 2.5-3.3 2.5-5.6l.3-6.7 3.1-58.6-3.3-162.8c-.3-2.8-1.7-5.3-3.9-6.7zm-111.4 22.5c-3.1 0-5.8 2.8-5.8 6.1l-4.4 140.6 4.4 67.2c.3 3.3 2.8 5.8 5.8 5.8 3.3 0 5.8-2.5 6.1-5.8l5-67.2-5-140.6c-.2-3.3-2.7-6.1-6.1-6.1zm376.7 62.8c-10.8 0-21.1 2.2-30.6 6.1-6.4-70.8-65.8-126.4-138.3-126.4-17.8 0-35 3.3-50.3 9.4-6.1 2.2-7.8 4.4-7.8 9.2v249.7c0 5 3.9 8.6 8.6 9.2h218.3c43.3 0 78.6-35 78.6-78.3.1-43.6-35.2-78.9-78.5-78.9zm-296.7-60.3c-4.2 0-7.5 3.3-7.8 7.8l-3.3 136.7 3.3 65.6c.3 4.2 3.6 7.5 7.8 7.5 4.2 0 7.5-3.3 7.5-7.5l3.9-65.6-3.9-136.7c-.3-4.5-3.3-7.8-7.5-7.8zm-53.6-7.8c-3.3 0-6.4 3.1-6.4 6.7l-3.9 145.3 3.9 66.9c.3 3.6 3.1 6.4 6.4 6.4 3.6 0 6.4-2.8 6.7-6.4l4.4-66.9-4.4-145.3c-.3-3.6-3.1-6.7-6.7-6.7zm26.7 3.4c-3.9 0-6.9 3.1-6.9 6.9L227 321.3l3.9 66.4c.3 3.9 3.1 6.9 6.9 6.9s6.9-3.1 6.9-6.9l4.2-66.4-4.2-141.7c0-3.9-3-6.9-6.9-6.9z" />
                                        </svg>
                                        <!-- /soundcloud icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 512 512">
                                            <path
                                                d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm50.7-186.9L162.4 380.6c-19.4 7.5-38.5-11.6-31-31l55.5-144.3c3.3-8.5 9.9-15.1 18.4-18.4l144.3-55.5c19.4-7.5 38.5 11.6 31 31L325.1 306.7c-3.2 8.5-9.9 15.1-18.4 18.4zM288 256a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" />
                                        </svg>
                                        <!-- /compass icn -->


                                    </span>


                                </div>

                            </div>
                        </div>
                    </div>
                </div>
                <div class="row justify-content-space-around align-items-center g-4 py-1">
                    <div class="col-4">

                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/gallery4.jpg" alt="">

                            <div class="card-body">
                                <h5 class="card-title">Music makes strong beats</h5>
                                <div class="card-text d-flex">Stream On:
                                    <span class="d-flex">

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" class="bi bi-spotify"
                                            viewBox="0 0 16 16">
                                            <path
                                                d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm3.669 11.538a.498.498 0 0 1-.686.165c-1.879-1.147-4.243-1.407-7.028-.77a.499.499 0 0 1-.222-.973c3.048-.696 5.662-.397 7.77.892a.5.5 0 0 1 .166.686zm.979-2.178a.624.624 0 0 1-.858.205c-2.15-1.321-5.428-1.704-7.972-.932a.625.625 0 0 1-.362-1.194c2.905-.881 6.517-.454 8.986 1.063a.624.624 0 0 1 .206.858zm.084-2.268C10.154 5.56 5.9 5.419 3.438 6.166a.748.748 0 1 1-.434-1.432c2.825-.857 7.523-.692 10.492 1.07a.747.747 0 1 1-.764 1.288z" />
                                        </svg>
                                        <!-- /spotify icn -->

                                        <svg class="thebase-svg-icon thebase-youtube-svg" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 28 28">
                                            <title>YouTube</title>
                                            <path
                                                d="M11.109 17.625l7.562-3.906-7.562-3.953v7.859zM14 4.156c5.891 0 9.797 0.281 9.797 0.281 0.547 0.063 1.75 0.063 2.812 1.188 0 0 0.859 0.844 1.109 2.781 0.297 2.266 0.281 4.531 0.281 4.531v2.125s0.016 2.266-0.281 4.531c-0.25 1.922-1.109 2.781-1.109 2.781-1.062 1.109-2.266 1.109-2.812 1.172 0 0-3.906 0.297-9.797 0.297v0c-7.281-0.063-9.516-0.281-9.516-0.281-0.625-0.109-2.031-0.078-3.094-1.188 0 0-0.859-0.859-1.109-2.781-0.297-2.266-0.281-4.531-0.281-4.531v-2.125s-0.016-2.266 0.281-4.531c0.25-1.937 1.109-2.781 1.109-2.781 1.062-1.125 2.266-1.125 2.812-1.188 0 0 3.906-0.281 9.797-0.281v0z">
                                            </path>
                                        </svg>
                                        <!-- /youtube icn -->

                                        <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 372.12 396.03">
                                            <path
                                                d="M370.43,0H372q0,67.4,0,134.8c0,48.13.13,96.26,0,144.39-.15,42.61-34.59,75-76.53,72.25a71.27,71.27,0,0,1-65.93-81.9c4.09-26.63,19-45.65,43.77-55.77,25.61-10.44,49.86-6.43,71.91,10.38.76.58,1.6,1.08,2.64,1.77v-117c-1.46.3-2.75.51-4,.84q-101.61,25.77-203.2,51.56c-6.11,1.55-6.1,1.58-6.1,8,0,55.46-.08,110.93,0,166.39,0,17.59-4.82,33.42-16,47.05-3.91,4.78-8.67,8.87-13,13.27H20.8c-8.53-6.29-15.21-14.26-20.8-23.2v-70.4a25,25,0,0,0,2.19-2.67C20.84,269.92,59,257.92,91.11,272.12c6.7,3,12.83,7.23,19.36,11V66.49c1.73-.52,3.23-1,4.76-1.41Q227.84,36.53,340.46,8C350.48,5.46,360.44,2.67,370.43,0Z">
                                            </path>
                                        </svg>
                                        <!-- /note icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 640 512">
                                            <path
                                                d="M111.4 256.3l5.8 65-5.8 68.3c-.3 2.5-2.2 4.4-4.4 4.4s-4.2-1.9-4.2-4.4l-5.6-68.3 5.6-65c0-2.2 1.9-4.2 4.2-4.2 2.2 0 4.1 2 4.4 4.2zm21.4-45.6c-2.8 0-4.7 2.2-5 5l-5 105.6 5 68.3c.3 2.8 2.2 5 5 5 2.5 0 4.7-2.2 4.7-5l5.8-68.3-5.8-105.6c0-2.8-2.2-5-4.7-5zm25.5-24.1c-3.1 0-5.3 2.2-5.6 5.3l-4.4 130 4.4 67.8c.3 3.1 2.5 5.3 5.6 5.3 2.8 0 5.3-2.2 5.3-5.3l5.3-67.8-5.3-130c0-3.1-2.5-5.3-5.3-5.3zM7.2 283.2c-1.4 0-2.2 1.1-2.5 2.5L0 321.3l4.7 35c.3 1.4 1.1 2.5 2.5 2.5s2.2-1.1 2.5-2.5l5.6-35-5.6-35.6c-.3-1.4-1.1-2.5-2.5-2.5zm23.6-21.9c-1.4 0-2.5 1.1-2.5 2.5l-6.4 57.5 6.4 56.1c0 1.7 1.1 2.8 2.5 2.8s2.5-1.1 2.8-2.5l7.2-56.4-7.2-57.5c-.3-1.4-1.4-2.5-2.8-2.5zm25.3-11.4c-1.7 0-3.1 1.4-3.3 3.3L47 321.3l5.8 65.8c.3 1.7 1.7 3.1 3.3 3.1 1.7 0 3.1-1.4 3.1-3.1l6.9-65.8-6.9-68.1c0-1.9-1.4-3.3-3.1-3.3zm25.3-2.2c-1.9 0-3.6 1.4-3.6 3.6l-5.8 70 5.8 67.8c0 2.2 1.7 3.6 3.6 3.6s3.6-1.4 3.9-3.6l6.4-67.8-6.4-70c-.3-2.2-2-3.6-3.9-3.6zm241.4-110.9c-1.1-.8-2.8-1.4-4.2-1.4-2.2 0-4.2.8-5.6 1.9-1.9 1.7-3.1 4.2-3.3 6.7v.8l-3.3 176.7 1.7 32.5 1.7 31.7c.3 4.7 4.2 8.6 8.9 8.6s8.6-3.9 8.6-8.6l3.9-64.2-3.9-177.5c-.4-3-2-5.8-4.5-7.2zm-26.7 15.3c-1.4-.8-2.8-1.4-4.4-1.4s-3.1.6-4.4 1.4c-2.2 1.4-3.6 3.9-3.6 6.7l-.3 1.7-2.8 160.8s0 .3 3.1 65.6v.3c0 1.7.6 3.3 1.7 4.7 1.7 1.9 3.9 3.1 6.4 3.1 2.2 0 4.2-1.1 5.6-2.5 1.7-1.4 2.5-3.3 2.5-5.6l.3-6.7 3.1-58.6-3.3-162.8c-.3-2.8-1.7-5.3-3.9-6.7zm-111.4 22.5c-3.1 0-5.8 2.8-5.8 6.1l-4.4 140.6 4.4 67.2c.3 3.3 2.8 5.8 5.8 5.8 3.3 0 5.8-2.5 6.1-5.8l5-67.2-5-140.6c-.2-3.3-2.7-6.1-6.1-6.1zm376.7 62.8c-10.8 0-21.1 2.2-30.6 6.1-6.4-70.8-65.8-126.4-138.3-126.4-17.8 0-35 3.3-50.3 9.4-6.1 2.2-7.8 4.4-7.8 9.2v249.7c0 5 3.9 8.6 8.6 9.2h218.3c43.3 0 78.6-35 78.6-78.3.1-43.6-35.2-78.9-78.5-78.9zm-296.7-60.3c-4.2 0-7.5 3.3-7.8 7.8l-3.3 136.7 3.3 65.6c.3 4.2 3.6 7.5 7.8 7.5 4.2 0 7.5-3.3 7.5-7.5l3.9-65.6-3.9-136.7c-.3-4.5-3.3-7.8-7.5-7.8zm-53.6-7.8c-3.3 0-6.4 3.1-6.4 6.7l-3.9 145.3 3.9 66.9c.3 3.6 3.1 6.4 6.4 6.4 3.6 0 6.4-2.8 6.7-6.4l4.4-66.9-4.4-145.3c-.3-3.6-3.1-6.7-6.7-6.7zm26.7 3.4c-3.9 0-6.9 3.1-6.9 6.9L227 321.3l3.9 66.4c.3 3.9 3.1 6.9 6.9 6.9s6.9-3.1 6.9-6.9l4.2-66.4-4.2-141.7c0-3.9-3-6.9-6.9-6.9z" />
                                        </svg>
                                        <!-- /soundcloud icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 512 512">
                                            <path
                                                d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm50.7-186.9L162.4 380.6c-19.4 7.5-38.5-11.6-31-31l55.5-144.3c3.3-8.5 9.9-15.1 18.4-18.4l144.3-55.5c19.4-7.5 38.5 11.6 31 31L325.1 306.7c-3.2 8.5-9.9 15.1-18.4 18.4zM288 256a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" />
                                        </svg>
                                        <!-- /compass icn -->


                                    </span>


                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="col-4">

                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/gallery5.jpg" alt="">

                            <div class="card-body">
                                <h5 class="card-title">Sound is the spark of time</h5>
                                <div class="card-text d-flex">Stream On:
                                    <span class="d-flex">

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" class="bi bi-spotify"
                                            viewBox="0 0 16 16">
                                            <path
                                                d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm3.669 11.538a.498.498 0 0 1-.686.165c-1.879-1.147-4.243-1.407-7.028-.77a.499.499 0 0 1-.222-.973c3.048-.696 5.662-.397 7.77.892a.5.5 0 0 1 .166.686zm.979-2.178a.624.624 0 0 1-.858.205c-2.15-1.321-5.428-1.704-7.972-.932a.625.625 0 0 1-.362-1.194c2.905-.881 6.517-.454 8.986 1.063a.624.624 0 0 1 .206.858zm.084-2.268C10.154 5.56 5.9 5.419 3.438 6.166a.748.748 0 1 1-.434-1.432c2.825-.857 7.523-.692 10.492 1.07a.747.747 0 1 1-.764 1.288z" />
                                        </svg>
                                        <!-- /spotify icn -->

                                        <svg class="thebase-svg-icon thebase-youtube-svg" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 28 28">
                                            <title>YouTube</title>
                                            <path
                                                d="M11.109 17.625l7.562-3.906-7.562-3.953v7.859zM14 4.156c5.891 0 9.797 0.281 9.797 0.281 0.547 0.063 1.75 0.063 2.812 1.188 0 0 0.859 0.844 1.109 2.781 0.297 2.266 0.281 4.531 0.281 4.531v2.125s0.016 2.266-0.281 4.531c-0.25 1.922-1.109 2.781-1.109 2.781-1.062 1.109-2.266 1.109-2.812 1.172 0 0-3.906 0.297-9.797 0.297v0c-7.281-0.063-9.516-0.281-9.516-0.281-0.625-0.109-2.031-0.078-3.094-1.188 0 0-0.859-0.859-1.109-2.781-0.297-2.266-0.281-4.531-0.281-4.531v-2.125s-0.016-2.266 0.281-4.531c0.25-1.937 1.109-2.781 1.109-2.781 1.062-1.125 2.266-1.125 2.812-1.188 0 0 3.906-0.281 9.797-0.281v0z">
                                            </path>
                                        </svg>
                                        <!-- /youtube icn -->

                                        <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 372.12 396.03">
                                            <path
                                                d="M370.43,0H372q0,67.4,0,134.8c0,48.13.13,96.26,0,144.39-.15,42.61-34.59,75-76.53,72.25a71.27,71.27,0,0,1-65.93-81.9c4.09-26.63,19-45.65,43.77-55.77,25.61-10.44,49.86-6.43,71.91,10.38.76.58,1.6,1.08,2.64,1.77v-117c-1.46.3-2.75.51-4,.84q-101.61,25.77-203.2,51.56c-6.11,1.55-6.1,1.58-6.1,8,0,55.46-.08,110.93,0,166.39,0,17.59-4.82,33.42-16,47.05-3.91,4.78-8.67,8.87-13,13.27H20.8c-8.53-6.29-15.21-14.26-20.8-23.2v-70.4a25,25,0,0,0,2.19-2.67C20.84,269.92,59,257.92,91.11,272.12c6.7,3,12.83,7.23,19.36,11V66.49c1.73-.52,3.23-1,4.76-1.41Q227.84,36.53,340.46,8C350.48,5.46,360.44,2.67,370.43,0Z">
                                            </path>
                                        </svg>
                                        <!-- /note icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 640 512">
                                            <path
                                                d="M111.4 256.3l5.8 65-5.8 68.3c-.3 2.5-2.2 4.4-4.4 4.4s-4.2-1.9-4.2-4.4l-5.6-68.3 5.6-65c0-2.2 1.9-4.2 4.2-4.2 2.2 0 4.1 2 4.4 4.2zm21.4-45.6c-2.8 0-4.7 2.2-5 5l-5 105.6 5 68.3c.3 2.8 2.2 5 5 5 2.5 0 4.7-2.2 4.7-5l5.8-68.3-5.8-105.6c0-2.8-2.2-5-4.7-5zm25.5-24.1c-3.1 0-5.3 2.2-5.6 5.3l-4.4 130 4.4 67.8c.3 3.1 2.5 5.3 5.6 5.3 2.8 0 5.3-2.2 5.3-5.3l5.3-67.8-5.3-130c0-3.1-2.5-5.3-5.3-5.3zM7.2 283.2c-1.4 0-2.2 1.1-2.5 2.5L0 321.3l4.7 35c.3 1.4 1.1 2.5 2.5 2.5s2.2-1.1 2.5-2.5l5.6-35-5.6-35.6c-.3-1.4-1.1-2.5-2.5-2.5zm23.6-21.9c-1.4 0-2.5 1.1-2.5 2.5l-6.4 57.5 6.4 56.1c0 1.7 1.1 2.8 2.5 2.8s2.5-1.1 2.8-2.5l7.2-56.4-7.2-57.5c-.3-1.4-1.4-2.5-2.8-2.5zm25.3-11.4c-1.7 0-3.1 1.4-3.3 3.3L47 321.3l5.8 65.8c.3 1.7 1.7 3.1 3.3 3.1 1.7 0 3.1-1.4 3.1-3.1l6.9-65.8-6.9-68.1c0-1.9-1.4-3.3-3.1-3.3zm25.3-2.2c-1.9 0-3.6 1.4-3.6 3.6l-5.8 70 5.8 67.8c0 2.2 1.7 3.6 3.6 3.6s3.6-1.4 3.9-3.6l6.4-67.8-6.4-70c-.3-2.2-2-3.6-3.9-3.6zm241.4-110.9c-1.1-.8-2.8-1.4-4.2-1.4-2.2 0-4.2.8-5.6 1.9-1.9 1.7-3.1 4.2-3.3 6.7v.8l-3.3 176.7 1.7 32.5 1.7 31.7c.3 4.7 4.2 8.6 8.9 8.6s8.6-3.9 8.6-8.6l3.9-64.2-3.9-177.5c-.4-3-2-5.8-4.5-7.2zm-26.7 15.3c-1.4-.8-2.8-1.4-4.4-1.4s-3.1.6-4.4 1.4c-2.2 1.4-3.6 3.9-3.6 6.7l-.3 1.7-2.8 160.8s0 .3 3.1 65.6v.3c0 1.7.6 3.3 1.7 4.7 1.7 1.9 3.9 3.1 6.4 3.1 2.2 0 4.2-1.1 5.6-2.5 1.7-1.4 2.5-3.3 2.5-5.6l.3-6.7 3.1-58.6-3.3-162.8c-.3-2.8-1.7-5.3-3.9-6.7zm-111.4 22.5c-3.1 0-5.8 2.8-5.8 6.1l-4.4 140.6 4.4 67.2c.3 3.3 2.8 5.8 5.8 5.8 3.3 0 5.8-2.5 6.1-5.8l5-67.2-5-140.6c-.2-3.3-2.7-6.1-6.1-6.1zm376.7 62.8c-10.8 0-21.1 2.2-30.6 6.1-6.4-70.8-65.8-126.4-138.3-126.4-17.8 0-35 3.3-50.3 9.4-6.1 2.2-7.8 4.4-7.8 9.2v249.7c0 5 3.9 8.6 8.6 9.2h218.3c43.3 0 78.6-35 78.6-78.3.1-43.6-35.2-78.9-78.5-78.9zm-296.7-60.3c-4.2 0-7.5 3.3-7.8 7.8l-3.3 136.7 3.3 65.6c.3 4.2 3.6 7.5 7.8 7.5 4.2 0 7.5-3.3 7.5-7.5l3.9-65.6-3.9-136.7c-.3-4.5-3.3-7.8-7.5-7.8zm-53.6-7.8c-3.3 0-6.4 3.1-6.4 6.7l-3.9 145.3 3.9 66.9c.3 3.6 3.1 6.4 6.4 6.4 3.6 0 6.4-2.8 6.7-6.4l4.4-66.9-4.4-145.3c-.3-3.6-3.1-6.7-6.7-6.7zm26.7 3.4c-3.9 0-6.9 3.1-6.9 6.9L227 321.3l3.9 66.4c.3 3.9 3.1 6.9 6.9 6.9s6.9-3.1 6.9-6.9l4.2-66.4-4.2-141.7c0-3.9-3-6.9-6.9-6.9z" />
                                        </svg>
                                        <!-- /soundcloud icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 512 512">
                                            <path
                                                d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm50.7-186.9L162.4 380.6c-19.4 7.5-38.5-11.6-31-31l55.5-144.3c3.3-8.5 9.9-15.1 18.4-18.4l144.3-55.5c19.4-7.5 38.5 11.6 31 31L325.1 306.7c-3.2 8.5-9.9 15.1-18.4 18.4zM288 256a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" />
                                        </svg>
                                        <!-- /compass icn -->


                                    </span>


                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="col-4">

                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/gallery6.jpg" alt="">

                            <div class="card-body">
                                <h5 class="card-title">make your night groovy</h5>
                                <div class="card-text d-flex">Stream On:
                                    <span class="d-flex">

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" class="bi bi-spotify"
                                            viewBox="0 0 16 16">
                                            <path
                                                d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm3.669 11.538a.498.498 0 0 1-.686.165c-1.879-1.147-4.243-1.407-7.028-.77a.499.499 0 0 1-.222-.973c3.048-.696 5.662-.397 7.77.892a.5.5 0 0 1 .166.686zm.979-2.178a.624.624 0 0 1-.858.205c-2.15-1.321-5.428-1.704-7.972-.932a.625.625 0 0 1-.362-1.194c2.905-.881 6.517-.454 8.986 1.063a.624.624 0 0 1 .206.858zm.084-2.268C10.154 5.56 5.9 5.419 3.438 6.166a.748.748 0 1 1-.434-1.432c2.825-.857 7.523-.692 10.492 1.07a.747.747 0 1 1-.764 1.288z" />
                                        </svg>
                                        <!-- /spotify icn -->

                                        <svg class="thebase-svg-icon thebase-youtube-svg" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 28 28">
                                            <title>YouTube</title>
                                            <path
                                                d="M11.109 17.625l7.562-3.906-7.562-3.953v7.859zM14 4.156c5.891 0 9.797 0.281 9.797 0.281 0.547 0.063 1.75 0.063 2.812 1.188 0 0 0.859 0.844 1.109 2.781 0.297 2.266 0.281 4.531 0.281 4.531v2.125s0.016 2.266-0.281 4.531c-0.25 1.922-1.109 2.781-1.109 2.781-1.062 1.109-2.266 1.109-2.812 1.172 0 0-3.906 0.297-9.797 0.297v0c-7.281-0.063-9.516-0.281-9.516-0.281-0.625-0.109-2.031-0.078-3.094-1.188 0 0-0.859-0.859-1.109-2.781-0.297-2.266-0.281-4.531-0.281-4.531v-2.125s-0.016-2.266 0.281-4.531c0.25-1.937 1.109-2.781 1.109-2.781 1.062-1.125 2.266-1.125 2.812-1.188 0 0 3.906-0.281 9.797-0.281v0z">
                                            </path>
                                        </svg>
                                        <!-- /youtube icn -->

                                        <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 372.12 396.03">
                                            <path
                                                d="M370.43,0H372q0,67.4,0,134.8c0,48.13.13,96.26,0,144.39-.15,42.61-34.59,75-76.53,72.25a71.27,71.27,0,0,1-65.93-81.9c4.09-26.63,19-45.65,43.77-55.77,25.61-10.44,49.86-6.43,71.91,10.38.76.58,1.6,1.08,2.64,1.77v-117c-1.46.3-2.75.51-4,.84q-101.61,25.77-203.2,51.56c-6.11,1.55-6.1,1.58-6.1,8,0,55.46-.08,110.93,0,166.39,0,17.59-4.82,33.42-16,47.05-3.91,4.78-8.67,8.87-13,13.27H20.8c-8.53-6.29-15.21-14.26-20.8-23.2v-70.4a25,25,0,0,0,2.19-2.67C20.84,269.92,59,257.92,91.11,272.12c6.7,3,12.83,7.23,19.36,11V66.49c1.73-.52,3.23-1,4.76-1.41Q227.84,36.53,340.46,8C350.48,5.46,360.44,2.67,370.43,0Z">
                                            </path>
                                        </svg>
                                        <!-- /note icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 640 512">
                                            <path
                                                d="M111.4 256.3l5.8 65-5.8 68.3c-.3 2.5-2.2 4.4-4.4 4.4s-4.2-1.9-4.2-4.4l-5.6-68.3 5.6-65c0-2.2 1.9-4.2 4.2-4.2 2.2 0 4.1 2 4.4 4.2zm21.4-45.6c-2.8 0-4.7 2.2-5 5l-5 105.6 5 68.3c.3 2.8 2.2 5 5 5 2.5 0 4.7-2.2 4.7-5l5.8-68.3-5.8-105.6c0-2.8-2.2-5-4.7-5zm25.5-24.1c-3.1 0-5.3 2.2-5.6 5.3l-4.4 130 4.4 67.8c.3 3.1 2.5 5.3 5.6 5.3 2.8 0 5.3-2.2 5.3-5.3l5.3-67.8-5.3-130c0-3.1-2.5-5.3-5.3-5.3zM7.2 283.2c-1.4 0-2.2 1.1-2.5 2.5L0 321.3l4.7 35c.3 1.4 1.1 2.5 2.5 2.5s2.2-1.1 2.5-2.5l5.6-35-5.6-35.6c-.3-1.4-1.1-2.5-2.5-2.5zm23.6-21.9c-1.4 0-2.5 1.1-2.5 2.5l-6.4 57.5 6.4 56.1c0 1.7 1.1 2.8 2.5 2.8s2.5-1.1 2.8-2.5l7.2-56.4-7.2-57.5c-.3-1.4-1.4-2.5-2.8-2.5zm25.3-11.4c-1.7 0-3.1 1.4-3.3 3.3L47 321.3l5.8 65.8c.3 1.7 1.7 3.1 3.3 3.1 1.7 0 3.1-1.4 3.1-3.1l6.9-65.8-6.9-68.1c0-1.9-1.4-3.3-3.1-3.3zm25.3-2.2c-1.9 0-3.6 1.4-3.6 3.6l-5.8 70 5.8 67.8c0 2.2 1.7 3.6 3.6 3.6s3.6-1.4 3.9-3.6l6.4-67.8-6.4-70c-.3-2.2-2-3.6-3.9-3.6zm241.4-110.9c-1.1-.8-2.8-1.4-4.2-1.4-2.2 0-4.2.8-5.6 1.9-1.9 1.7-3.1 4.2-3.3 6.7v.8l-3.3 176.7 1.7 32.5 1.7 31.7c.3 4.7 4.2 8.6 8.9 8.6s8.6-3.9 8.6-8.6l3.9-64.2-3.9-177.5c-.4-3-2-5.8-4.5-7.2zm-26.7 15.3c-1.4-.8-2.8-1.4-4.4-1.4s-3.1.6-4.4 1.4c-2.2 1.4-3.6 3.9-3.6 6.7l-.3 1.7-2.8 160.8s0 .3 3.1 65.6v.3c0 1.7.6 3.3 1.7 4.7 1.7 1.9 3.9 3.1 6.4 3.1 2.2 0 4.2-1.1 5.6-2.5 1.7-1.4 2.5-3.3 2.5-5.6l.3-6.7 3.1-58.6-3.3-162.8c-.3-2.8-1.7-5.3-3.9-6.7zm-111.4 22.5c-3.1 0-5.8 2.8-5.8 6.1l-4.4 140.6 4.4 67.2c.3 3.3 2.8 5.8 5.8 5.8 3.3 0 5.8-2.5 6.1-5.8l5-67.2-5-140.6c-.2-3.3-2.7-6.1-6.1-6.1zm376.7 62.8c-10.8 0-21.1 2.2-30.6 6.1-6.4-70.8-65.8-126.4-138.3-126.4-17.8 0-35 3.3-50.3 9.4-6.1 2.2-7.8 4.4-7.8 9.2v249.7c0 5 3.9 8.6 8.6 9.2h218.3c43.3 0 78.6-35 78.6-78.3.1-43.6-35.2-78.9-78.5-78.9zm-296.7-60.3c-4.2 0-7.5 3.3-7.8 7.8l-3.3 136.7 3.3 65.6c.3 4.2 3.6 7.5 7.8 7.5 4.2 0 7.5-3.3 7.5-7.5l3.9-65.6-3.9-136.7c-.3-4.5-3.3-7.8-7.5-7.8zm-53.6-7.8c-3.3 0-6.4 3.1-6.4 6.7l-3.9 145.3 3.9 66.9c.3 3.6 3.1 6.4 6.4 6.4 3.6 0 6.4-2.8 6.7-6.4l4.4-66.9-4.4-145.3c-.3-3.6-3.1-6.7-6.7-6.7zm26.7 3.4c-3.9 0-6.9 3.1-6.9 6.9L227 321.3l3.9 66.4c.3 3.9 3.1 6.9 6.9 6.9s6.9-3.1 6.9-6.9l4.2-66.4-4.2-141.7c0-3.9-3-6.9-6.9-6.9z" />
                                        </svg>
                                        <!-- /soundcloud icn -->

                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10"
                                            viewBox="0 0 512 512">
                                            <path
                                                d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm50.7-186.9L162.4 380.6c-19.4 7.5-38.5-11.6-31-31l55.5-144.3c3.3-8.5 9.9-15.1 18.4-18.4l144.3-55.5c19.4-7.5 38.5 11.6 31 31L325.1 306.7c-3.2 8.5-9.9 15.1-18.4 18.4zM288 256a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z" />
                                        </svg>
                                        <!-- /compass icn -->


                                    </span>


                                </div>

                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>

        <div id="album_promo">
            <div class="bg_promo p-5 bg-black">
                <div class="container d-flex text-start py-5">
                    <div class="flex-start">
                        <p>Instrumental rock</p>
                        <h1 class="">Album out now</h1>
                        <button type="button" class="btn btn_cstm btn-lg">read more</button>

                    </div>
                </div>
            </div>
        </div>

        <div id="numbers_section">
            <div class="bg_numbers p-5">
                <div class="container py-5">

                    <div class="row d-flex justify-content-center">
                        <div class="col-3">
                            <div class="award">
                                <span>
                                    <svg data-name="Layer 1" height="30" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 261.58 393.53">
                                        <path
                                            d="M99.27,393.53c-2.63-1.26-5.49-2.18-7.84-3.83C85,385.18,83,378.64,83,371q.14-114.89.13-229.78A12.33,12.33,0,0,0,80,132.6q-37.2-44-74.22-88.18C-3,34-1.6,20.69,9,13.75,17.47,8.23,28.11,10,35.24,18.26c6.6,7.66,13,15.47,19.54,23.22,15.4,18.34,30.89,36.6,46.15,55.06,4.62,5.59,10.09,8.22,17.33,8,8.45-.27,16.92-.25,25.36,0,7.12.21,12.44-2.5,17-7.94q32.46-39,65.2-77.69c7.31-8.69,17.74-10.75,26.4-5.38,10.64,6.6,12.54,20.05,4.13,30.28-7.89,9.59-15.94,19.05-23.93,28.57C215.2,92.87,198,113.34,180.88,134a11,11,0,0,0-2.28,6.47c-.11,76-.24,151.91.05,227.86,0,12.57-4,21.32-16.36,25.25h-7.68c-12.68-4.26-16.29-13.62-16.16-26.24.4-36.08.14-72.17.14-108.25v-4.4H123v4.79q0,55.85,0,111.71c0,9.05-3.14,16.21-11.56,20.42-1.45.73-3,1.32-4.47,2Z">
                                        </path>
                                        <path
                                            d="M136.16,0c7.89,1.81,15.4,4.41,21.85,9.6A43.4,43.4,0,1,1,87.54,41.15C88.75,20.5,103.15,4.36,123.65.66A9.79,9.79,0,0,0,125.4,0Z">
                                        </path>
                                    </svg>
                                </span>
                                <h2 class="">163</h2>
                                <p>concerts</p>


                            </div>
                        </div>
                        <div class="col-3">
                            <div class="award">
                                <span><svg data-name="Layer 1" height="30" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 56.99 69.46">
                                        <path
                                            d="M0,43.44v-1c4.24-4.84,9-9.13,15.64-9.91,2.81-.32,3.89-1.41,4.86-3.9C26.62,13,39.59,5.47,55.05,0c3.42,16,3.05,30.87-7.24,44.08-1.56,2-2.27,3.49-1.17,6.17,2.69,6.53,1.78,13-1.54,19.21h-1L33,56.58c-7.34,3.23-13.66.22-15.68-6.68a4.81,4.81,0,0,0-2.85-2.68C9.71,45.79,4.84,44.67,0,43.44ZM39.55,31.79c4.33.13,7.69-2.85,7.39-6.39-.32-3.78-2.43-6.25-6.45-6.23s-6.33,2.11-6.42,6.12S36.28,31.35,39.55,31.79Z">
                                        </path>
                                    </svg></span>
                                <h2 class="">145</h2>
                                <p>happy clients</p>


                            </div>
                        </div>
                        <div class="col-3">
                            <div class="award">
                                <span><svg data-name="Layer 1" height="30" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 409.64 409.64">
                                        <path
                                            d="M0,98.41V36.8C5.58,23.23,9.67,20.53,24.7,20.53q26.13,0,52.26,0h4.65V0H328V20.53h4.64c18.09,0,36.18-.13,54.26.1a36.87,36.87,0,0,1,12,2.16c6.31,2.31,8.8,8.06,10.71,14V98.41a29.23,29.23,0,0,0-.77,3c-5.93,42-36,73.12-77.62,80.15-2.57.44-3.44,1.36-3.67,4-.74,8.47-1.07,17.09-2.93,25.34-8.91,39.36-31.72,68-68.1,85.49-7.83,3.77-16.3,6.22-24.86,9.42q6.9,20.77,14.06,42.27h4.51c13.46,0,26.92,0,40.38,0,10.66,0,17.28,4.73,20.76,14.72q6.45,18.47,12.83,36.94c1.12,3.24,2,6.56,3,9.85H82.41a3.6,3.6,0,0,1,0-1.18c5.61-16.21,11.06-32.49,17-48.59,2.18-5.95,7-9.68,13.3-11a47.22,47.22,0,0,1,9.52-.71c13.85-.07,27.69,0,41.69,0L178,305.77c-.51-.48-.67-.75-.89-.82-1.14-.35-2.3-.65-3.45-1q-52.69-14.62-78.1-63A119.39,119.39,0,0,1,82.1,186.4c0-3.36-1.3-4.3-4.5-4.94-35.76-7.19-60.46-27.58-72.71-62.19C2.52,112.58,1.59,105.38,0,98.41Zm266.21,24.53c-13.83,0-26.61-.09-39.4.07-2.71,0-4-.85-5.07-3.27-5-11.34-10.1-22.6-15.19-33.88-.46-1-1-2-1.81-3.37-.92,1.85-1.6,3.12-2.18,4.43-4.92,10.92-9.87,21.82-14.66,32.8-1.05,2.41-2.33,3.33-5.06,3.29-11.46-.17-22.92-.08-34.38-.06-1.39,0-2.78.13-4.69.22,1.1,1.56,1.81,2.64,2.59,3.68,8.46,11.31,16.88,22.65,25.45,33.87a7.2,7.2,0,0,1,1.38,6.94c-3.16,12.06-6.08,24.18-9.25,37,1.77-.91,2.92-1.48,4-2.09,11.33-6.21,22.68-12.36,33.94-18.69a5.07,5.07,0,0,1,5.78,0c10.08,5.7,20.27,11.22,30.42,16.79,2.29,1.26,4.62,2.47,7.38,3.94-.16-1.05-.2-1.55-.32-2-2.92-11.75-6-23.47-8.67-35.27a8.67,8.67,0,0,1,1.22-6.09c3.81-5.62,8.05-10.95,12.12-16.38ZM328,61.68v79.43c1.7-.39,3.13-.57,4.46-1,16.63-5.71,27.77-17.13,33.15-33.64,2.1-6.45,2.6-13.54,3-20.39.5-8,.12-16.15.12-24.37ZM81.58,141V61.81H40.9c0,8.65.09,17,0,25.37-.17,12.7,3.4,24.15,11.25,34.15A55.28,55.28,0,0,0,81.58,141Z">
                                        </path>
                                    </svg></span>
                                <h2 class="">72</h2>
                                <p>music awards</p>


                            </div>
                        </div>
                        <div class="col-3">
                            <div class="award">
                                <span><svg data-name="Layer 1" height="30" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 372.12 396.03">
                                        <path
                                            d="M370.43,0H372q0,67.4,0,134.8c0,48.13.13,96.26,0,144.39-.15,42.61-34.59,75-76.53,72.25a71.27,71.27,0,0,1-65.93-81.9c4.09-26.63,19-45.65,43.77-55.77,25.61-10.44,49.86-6.43,71.91,10.38.76.58,1.6,1.08,2.64,1.77v-117c-1.46.3-2.75.51-4,.84q-101.61,25.77-203.2,51.56c-6.11,1.55-6.1,1.58-6.1,8,0,55.46-.08,110.93,0,166.39,0,17.59-4.82,33.42-16,47.05-3.91,4.78-8.67,8.87-13,13.27H20.8c-8.53-6.29-15.21-14.26-20.8-23.2v-70.4a25,25,0,0,0,2.19-2.67C20.84,269.92,59,257.92,91.11,272.12c6.7,3,12.83,7.23,19.36,11V66.49c1.73-.52,3.23-1,4.76-1.41Q227.84,36.53,340.46,8C350.48,5.46,360.44,2.67,370.43,0Z">
                                        </path>
                                    </svg></span>
                                <h2 class="">182</h2>
                                <p>total songs</p>


                            </div>
                        </div>
                    </div>


                </div>
            </div>
        </div>

        <div id="blog_section" class="p-5">
            <div class="container">
                <div class="row justify-content-space-around align-items-center g-4 py-1">
                    <div class="col-4 text-start">

                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/hip_hop_wired_to_dance_make.jpg" alt="">

                            <div class="card-body text-start">
                                <h6 class="card-title">Hip hop wired to dance make</h6>
                                <div class="wrapper d-flex align-items-center">

                                    <div class="orange_filter mb-1"><svg xmlns="http://www.w3.org/2000/svg" height="8"
                                            viewBox="0 0 448 512">
                                            <path
                                                d="M128 0c17.7 0 32 14.3 32 32V64H288V32c0-17.7 14.3-32 32-32s32 14.3 32 32V64h48c26.5 0 48 21.5 48 48v48H0V112C0 85.5 21.5 64 48 64H96V32c0-17.7 14.3-32 32-32zM0 192H448V464c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V192zm64 80v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm128 0v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H208c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H336zM64 400v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H208zm112 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H336c-8.8 0-16 7.2-16 16z" />
                                        </svg></div>
                                    <div class="card-text d-flex ms-1">06 November 2021
                                    </div>
                                </div>
                                <p class="text-start">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Minus minima
                                    eaque culpa atque eos fugit totam assumenda architecto corrupti. Praesentium quis odit,
                                    et saepe adipisci voluptatem tempora dignissimos totam nemo?</p>

                            </div>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/music_business-worldwide.jpg" alt="">

                            <div class="card-body text-start">
                                <h6 class="card-title">Music business worlwide</h6>
                                <div class="wrapper d-flex align-items-center">

                                    <div class="orange_filter mb-1"><svg xmlns="http://www.w3.org/2000/svg" height="8"
                                            viewBox="0 0 448 512">
                                            <path
                                                d="M128 0c17.7 0 32 14.3 32 32V64H288V32c0-17.7 14.3-32 32-32s32 14.3 32 32V64h48c26.5 0 48 21.5 48 48v48H0V112C0 85.5 21.5 64 48 64H96V32c0-17.7 14.3-32 32-32zM0 192H448V464c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V192zm64 80v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm128 0v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H208c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H336zM64 400v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H208zm112 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H336c-8.8 0-16 7.2-16 16z" />
                                        </svg></div>
                                    <div class="card-text d-flex ms-1">27 May 2022
                                    </div>
                                </div>
                                <p class="text-start">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Minus minima
                                    eaque culpa atque eos fugit totam assumenda architecto corrupti. Praesentium quis odit,
                                    et saepe adipisci voluptatem tempora dignissimos totam nemo?</p>

                            </div>
                        </div>

                    </div>
                    <div class="col-4">

                        <div class="card rounded-0">

                            <img class="img-fluid" src="./assets/img/giveaways_rock.jpg" alt="">

                            <div class="card-body text-start">
                                <h6 class="card-title"> Giveaways rock to all</h6>
                                <div class="wrapper d-flex align-items-center">

                                    <div class="orange_filter mb-1"><svg xmlns="http://www.w3.org/2000/svg" height="8"
                                            viewBox="0 0 448 512">
                                            <path
                                                d="M128 0c17.7 0 32 14.3 32 32V64H288V32c0-17.7 14.3-32 32-32s32 14.3 32 32V64h48c26.5 0 48 21.5 48 48v48H0V112C0 85.5 21.5 64 48 64H96V32c0-17.7 14.3-32 32-32zM0 192H448V464c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V192zm64 80v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm128 0v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H208c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H336zM64 400v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H208zm112 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H336c-8.8 0-16 7.2-16 16z" />
                                        </svg></div>
                                    <div class="card-text d-flex ms-1">07 May 2022
                                    </div>
                                </div>
                                <p class="text-start">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Minus minima
                                    eaque culpa atque eos fugit totam assumenda architecto corrupti. Praesentium quis odit,
                                    et saepe adipisci voluptatem tempora dignissimos totam nemo?</p>

                            </div>
                        </div>

                    </div>
                </div>
            </div>

        </div>

    </main>

    <footer>
        <div class="footer_top d-flex">

            <div class="col">
                <img src="./assets/img/instagram_img1.jpg" alt="">
            </div>
            <div class="col">
                <img src="./assets/img/instagram_img2.jpg" alt="">
            </div>
            <div class="col">
                <img src="./assets/img/instagram_img3.jpg" alt="">
            </div>
            <div class="col">
                <img src="./assets/img/instagram_img4.jpg" alt="">
            </div>
            <div class="col">
                <img src="./assets/img/instagram_img5.jpg" alt="">
            </div>
            <div class="col">
                <img src="./assets/img/instagram_img6.jpg" alt="">
            </div>
            <div class="col">
                <img src="./assets/img/instagram_img7.jpg" alt="">
            </div>
            <div class="col">
                <img src="./assets/img/instagram_img8.jpg" alt="">
            </div>

        </div>

        <div class="footer_mid">
            <div class="container pt-5">
                <div class="row">
                    <div class="col-3">
                        <ul class="list-unstyled">
                            <h6>Bookings</h6>
                            <li>502 New Design Str, 
                                Melbourne, San Francisco, 
                                CA 94110, United States Of 
                                America Australia
                            </li>
                            <li>T: 1123-456-789</li>
                            <li>E-mail: demo@example.com</li>
                            <div>
                                <img src="" alt="">
                                <img src="" alt="">
                                <img src="" alt="">
                                <img src="" alt="">
                            </div>
                        </ul>
                    </div>
                    <div class="col-3">
                        <ul class="list-unstyled">
                            <h6>Information</h6>
                            <li>Product Support</li>
                            <li>Checkout</li>
                            <li>Report Abuse</li>
                            <li>Redeem Voucher</li>
                            <li>Order Status</li>
                        </ul>
                    </div>
                    <div class="col-3">
                        <ul class="list-unstyled">
                            <h6>Support</h6>
                            <li>Policies & Rules</li>
                            <li>Privacy Policy</li>
                            <li>License Policy</li>
                            <li>My account</li>
                            <li>Locality</li>
                        </ul>
                    </div>
                    <div class="col-3">
                        <h6>Newsletter</h6>
                        <form>
                            <div class="mb-3">
                                
                                <input type="email" class="form-control" placeholder="Email" required id="exampleInputEmail1"
                                    aria-describedby="emailHelp">
                            </div>
                            <div class="mb-3">
                            
                                <textarea class="form-control" id="validationTextarea"
                                    placeholder="Message" required></textarea>
                                <div class="invalid-feedback">
                                    Please enter a message in the textarea.
                                </div>
                            </div>
                            <button type="submit" class="btn btn_cstm">Send Message</button>
                        </form>
                    </div>
                    <div class="col"></div>
                </div>
            </div>
        </div>
        <div class="footer_bot">
            <hr>
            <p>2023 All Rights Reserved. Developed By
                <a href="#">TemplateMela</a>
            </p>
        </div>
    </footer>
</template>

<style lang="scss">
@use './assets/scss/partials/variables' as *;

body {
    font-family: 'Oswald', sans-serif;
}
</style>
